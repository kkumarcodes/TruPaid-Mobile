...
import {GET_FOLLOWING_FEED_QUERY, INFLUENCED_BY_QUERY} from '../../../utils/Query';
...
const FollowingFeed = (props) => {
  ...
  const {data: postData, loading, error, fetchMore} = useQuery(GET_FOLLOWING_FEED_QUERY, {
    variables: {
      'pageSize': limit,
      'next': nextCursor,
    },
  ...
  useEffect(() => {
    const posts = postData?.followingFeed?.nodes ? postData?.followingFeed?.nodes : [];
    if (posts?.length > 0 && !initData) {
      setPostList(posts);
      setNextCursor(postData?.followingFeed?.cursor);
  ...
  const loadMore = async () => {
    ...
    fetchMore({
      ...
      const posts = res?.data?.followingFeed?.nodes ? res?.data?.followingFeed?.nodes : [];
      setPostList([...postList, ...posts]);
      setNextCursor(res?.data?.followingFeed?.cursor);
      setIsNoData(posts?.length < limit);
